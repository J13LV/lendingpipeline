...
const PHASES = [
  ...
  { id: 3, label: "Processing", short: "PR", color: "#F5A623", bg: "#2e2210", stages: ["Full Application","Doc Collection","Title Ordered","Appraisal Ordered","Insurance Ordered"] },
...
const LOAN_TYPE_GROUPS = [
  ...
  { group: "NV — DPA", types: [
    "NV HIP Conventional","NV HIP FHA","NV Rural Home at Last FHA","NV Rural Home at Last Conv",
    "Chenoa Fund FHA","NHF Grant FHA","NHF Grant Conv","NHF Grant VA","NHF Grant USDA"
  ]},
  { group: "FL — DPA", types: [
...
const SAMPLE = [
  { id:"f1", borrower:"Ariel Villalobos", loan:385000, type:"NV HIP FHA", stage:"Condition Clearing", daysInStage:3, closing:"2026-04-14", note:"Waiting on updated pay stubs", closedAt:null },
  { id:"f2", borrower:"Maria Santos", loan:420000, type:"FL Hometown Heroes FHA", stage:"Appraisal Ordered", daysInStage:6, closing:"2026-04-28", note:"", closedAt:null },
  { id:"f3", borrower:"James Ortega", loan:295000, type:"VA IRRRL", stage:"Under Contract", daysInStage:2, closing:"2026-05-10", note:"Agent: Anamary APG", closedAt:null },
  ...
  { id:"f5", borrower:"Carlos Mendez", loan:340000, type:"TX TSAHC Heroes FHA", stage:"Pre-Qualification", daysInStage:0, closing:"2026-05-20", note:"Smart Bee referral", closedAt:null },
  { id:"f6", borrower:"Angela Torres", loan:275000, type:"NHF Grant FHA", stage:"CD Issued", daysInStage:1, closing:"2026-04-09", note:"3-day wait ends 4/9", closedAt:null },
  { id:"f7", borrower:"David Kim", loan:460000, type:"CE Second Expanded Access (ITIN)", stage:"Doc Collection", daysInStage:4, closing:"2026-05-02", note:"Self-employed — need 2yr biz returns", closedAt:null },
  { id:"f8", borrower:"Rosa Jimenez", loan:318000, type:"NV HIP FHA", stage:CLOSED_STAGE, daysInStage:0, closing:"2026-03-15", note:"Smooth close. Smart Bee referral.", closedAt:"2026-03-15" },
  { id:"f9", borrower:"Tony Reyes", loan:425000, type:"FL Hometown Heroes Conv", stage:CLOSED_STAGE, daysInStage:0, closing:"2026-03-28", note:"APG Realty. Requested Google review.", closedAt:"2026-03-28" },
];

const STORAGE_KEY = "delvalle_pipeline_v1";

function getPhase(s) {
  if (s === CLOSED_STAGE) return { id:99, label:"Closed", short:"✓", color:"#06D6A0", bg:"#00281e", stages:[CLOSED_STAGE] };
  return PHASES.find(p => p.stages.includes(s)) || PHASES[0];
}
    ...
    <div style={{ fontFamily:"'DM Mono','Courier New',monospace", background:"#0D1117", minHeight:"100vh", color:"#E6EDF3" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        select optgroup{background:#0D1117;color:#484F58;font-style:normal;}
      `}</style>
      ...
      <div style={{ background:"#161B22", borderBottom:"1px solid #21262D", padding:"14px 24px", display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
        <div>
          ...
          <div style={{ fontSize:10, color:"#484F58", letterSpacing:"2px", marginTop:1 }}>MORTGAGE BY DELVALLE · PRMG 541-A</div>
        </div>
      ...
      <div style={{ background:"#161B22", borderBottom:"1px solid #21262D", padding:"10px 24px", display:"flex", gap:8, alignItems:"center", overflowX:"auto" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:12, fontSize:10, color:"#484F58", whiteSpace:"nowrap" }}>
          ...
          <span style={{ color:"#F5A623" }}>● WARN ≤7d</span>
      ...
      <div style={{ padding:"20px 24px" }}>
        ...
        {view==="closed"&&<div className="fi">
          ...
          {display.length===0
            ? <div style={{ padding:40, textAlign:"center", color:"#30363D", fontSize:13 }}>No closed files yet.</div>
                        ...
                        return (
                          <div key={f.id} className="card" onClick={()=>setDetail(f)}
                            ...
                            <div style={{ display:"flex", gap:6, marginTop:2 }}>
                              ...
                              <button className="hov" onClick={e=>{e.stopPropagation();if(window.confirm(`Mark ${f.borrower} as CLOSED?`))closeFile(f.id);}}
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          ...
          {isClosed?(
            ...
            <>
              ...
              <button className="hov" onClick={()=>{if(window.confirm(`Close ${file.borrower}?`))onCloseFile();}}
          ...
          <button className="hov" onClick={()=>{if(window.confirm("Delete permanently?"))onDelete();}}
...
function AddModal({onClose,onAdd}){
  ...
  const [type,setType]=useState("NV HIP FHA");
      ...
      <div className="fi" style={{background:"#161B22",border:"1px solid #30363D",borderRadius:12,padding:24,width:"100%",maxWidth:440,display:"flex",flexDirection:"column",gap:14}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <div style={{gridColumn:"1/-1"}}>
            ...
            <input value={borrower} onChange={e=>setBorrower(e.target.value)} placeholder="Full legal name" style={IS}/>
          </div>
          <div>
            <div style={{fontSize:10,color:"#484F58",letterSpacing:"1px",marginBottom:5}}>LOAN AMOUNT</div>
          ...
          <div style={{gridColumn:"1/-1"}}>
            <div style={{fontSize:10,color:"#484F58",letterSpacing:"1px",marginBottom:5}}>NOTES</div>
            <input value={note} onChange={e=>setNote(e.target.value)} placeholder="Lead source, agent, key info..." style={IS}/>
          </div>
        </div>
